...
def load_promocodes_list():
    if os.path.exists(PROMOCODES_FILE):
        with open(PROMOCODES_FILE, 'r', encoding='utf-8') as f:
            return json.load(f)
    default_promocodes = {
        'ZETTA10': {'discount': 10, 'type': 'percent', 'active': True, 'applicable_categories': ['all']},
        'ZETTA20': {'discount': 20, 'type': 'percent', 'active': True, 'applicable_categories': ['all']},
        'ZETTA1000': {'discount': 1000, 'type': 'fixed', 'active': True, 'applicable_categories': ['all']},
        'ZETTA15': {'discount': 15, 'type': 'percent', 'active': True,
                    'applicable_categories': ['services', 'pc_build']},
    }
...
def is_promocode_applicable(promo_data, cart_items):
    applicable_categories = promo_data.get('applicable_categories', ['all'])
...
OFFICE_COORDINATES = {
    'lat': 53.3543,
    'lon': 83.7493,
    'address': 'г. Барнаул, ул. Юрина, 182'
}
...
def apply_promo():
    ...
    if promo_code in promocodes and promocodes[promo_code].get('active', True):
        ...
        for product_id, quantity in cart.items():
            ...
            if product:
                cart_items.append({'category': product.get('category')})

        if not is_promocode_applicable(promo_data, cart_items):
            session.pop('promo_code', None)
            return jsonify({'success': False, 'message': 'Этот промокод не применим к товарам в вашей корзине'})
...
def get_cart():
    ...
    if promo_code and promo_code in promocodes and promocodes[promo_code].get('active', True):
        ...
        if is_promocode_applicable(promo, items):
...
def admin_add_promocode():
    ...
    applicable_categories = data.get('applicable_categories', ['all'])
...
def admin_edit_promocode():
    ...
    promocodes = load_promocodes_list()
    if code in promocodes:
        promocodes[code]['applicable_categories'] = applicable_categories
        save_promocodes_list(promocodes)
        return jsonify({'success': True})
...
def admin_remove_admin():
    ...
    if email == session['user_email']:
        return jsonify({'success': False, 'message': 'Нельзя снять права администратора с самого себя'})

    users = load_users()
    email_lower = email.lower()
    if email_lower in users:
        users[email_lower]['is_admin'] = False
        save_users(users)
        return jsonify({'success': True})
    return jsonify({'success': False, 'message': 'Пользователь не найден'})
...
<head>
    ...
    <style>
        * {
        ...
        @keyframes lightning {
            0% {
                text-shadow: 0 0 2px #fff, 0 0 5px #ff0, 0 0 10px #ff0;
                transform: scale(1);
            }
            20% {
            ...
            40% {
                text-shadow: 0 0 3px #fff, 0 0 8px #ff0, 0 0 12px #ff0;
                transform: scale(1);
            }
            60% {
                text-shadow: 0 0 8px #fff, 0 0 15px #ffaa00, 0 0 25px #ffaa00;
                transform: scale(1.1);
            }
            80% {
                text-shadow: 0 0 2px #fff, 0 0 5px #ff0, 0 0 8px #ff0;
                transform: scale(1);
            }
            100% {
                text-shadow: 0 0 2px #fff, 0 0 5px #ff0, 0 0 10px #ff0;
...
<body>
    <div class="header">
        <div class="header-content">
            <div class="logo" onclick="goToHome()">
                <span class="logo-icon lightning-animation">⚡</span>
                ...
                <div id="adminNews" class="admin-section">
                    <div class="admin-form">
                        ...
                        <form id="addNewsForm" enctype="multipart/form-data">
                            ...
                            <input type="text" id="newsVideoUrl" placeholder="Ссылка на видео (YouTube/Vimeo)">
                ...
                <div id="adminPromocodes" class="admin-section">
                    <div class="admin-form">
                        ...
                        <form id="addPromocodeForm">
                            ...
                            <select id="promocodeCategories" multiple size="3">
                                <option value="all">Все товары</option>
                                <option value="computers">Компьютеры</option>
                                <option value="services">Услуги</option>
                                <option value="pc_build">Сборка компьютера</option>
                                <option value="websites">Сайты</option>
                                <option value="components">Комплектующие</option>
                            </select>
                            <small style="color:#888;">Удерживайте Ctrl для выбора нескольких категорий</small>
        ...
        <div class="news-modal-content">
            ...
            <div class="news-modal-body">
                <form id="addNewsFormModal" enctype="multipart/form-data">
                    ...
                    <input type="text" id="modalNewsVideoUrl" class="form-input" style="margin-bottom: 1rem;" placeholder="Ссылка на видео (YouTube/Vimeo)">
    ...
    <script>
            ...
            if (item.video_url && item.video_url.trim()) {
                ...
                if (item.video_url.includes('youtube.com') || item.video_url.includes('youtu.be')) {
                    ...
                    if (item.video_url.includes('youtu.be')) {
                        videoId = item.video_url.split('/').pop();
                    } else if (item.video_url.includes('watch?v=')) {
                        videoId = item.video_url.split('v=')[1].split('&')[0];
                    }
                    if (videoId) {
                        videoHtml = `<iframe class="news-modal-video" width="100%" height="315" src="https://www.youtube.com/embed/${videoId}" frameborder="0" allowfullscreen></iframe>`;
                    }
                } else if (item.video_url.includes('vimeo.com')) {
                    const vimeoId = item.video_url.split('/').pop();
                    videoHtml = `<iframe class="news-modal-video" width="100%" height="315" src="https://player.vimeo.com/video/${vimeoId}" frameborder="0" allowfullscreen></iframe>`;
                } else {
                    videoHtml = `<video class="news-modal-video" width="100%" height="auto" controls src="${item.video_url}"></video>`;
                }
                mediaContainer.innerHTML = videoHtml;
            } else {
                mediaContainer.innerHTML = `<img id="newsModalImage" class="news-modal-image" src="${item.image}" alt="${item.title}">`;
            }
        ...
        function submitAddNews() {
            ...
            formData.append('video_url', document.getElementById('modalNewsVideoUrl').value);
        ...
        function editNewsFromCarousel(id) {
            ...
            const newVideoUrl = prompt('Введите ссылку на видео (или оставьте пустым):', news.video_url || '');

            fetch('/api/admin/edit-news', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({id: id, title: newTitle, text: newText, fullText: newFullText, video_url: newVideoUrl})
            }).then(res => res.json()).then(data => {
                if (data.success) {
        ...
        function renderCarousel() {
            ...
            for (let i = 0; i < totalSlides; i++) {
                ...
                for (let j = startIdx; j < endIdx; j++) {
                    ...
                    let mediaHtml = '';
                    if (item.video_url && item.video_url.trim()) {
                        mediaHtml = `<video class="news-card-video" src="${item.video_url}" muted loop autoplay playsinline onmouseover="this.play()" onmouseout="this.pause()"></video>`;
                    } else {
                        mediaHtml = `<img src="${item.image}" class="news-card-image" alt="${item.title}" onerror="this.src='https://via.placeholder.com/300x200/2c3e50/ffffff?text=No+Image'">`;
                    }
                            ...
                            ${news.map(n => `
                                <div class="admin-news-card">
                                    <div class="admin-news-info">
                                        ...
                                        ${n.video_url ? `<div class="admin-news-date">🎬 Видео: ${n.video_url}</div>` : ''}
                                    </div>
                            ...
                            ${promocodesArray.map(([code, data]) => `
                                <div class="admin-promocode-card">
                                    <div class="admin-promocode-info">
                                        ...
                                        <div class="admin-promocode-details">
                                            Категории: ${data.applicable_categories ? data.applicable_categories.join(', ') : 'all'}
                                        </div>
                                    </div>
        ...
        function editPromocode(code) {
            const categoriesInput = prompt('Введите категории через запятую (all, computers, services, pc_build, websites, components):', 'all');
            if (categoriesInput !== null) {
                const categories = categoriesInput.split(',').map(c => c.trim().toLowerCase());
                fetch('/api/admin/edit-promocode', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({code: code, applicable_categories: categories})
                }).then(res => res.json()).then(data => {
                    if (data.success) {
        ...
        function addPromocode() {
            ...
            const categorySelect = document.getElementById('promocodeCategories');
            ...
            fetch('/api/admin/add-promocode', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({code: code, type: type, discount: discount, applicable_categories: selectedCategories})
            }).then(res => res.json()).then(data => {
                if (data.success) {
                ...
                .then(promocodes => {
                    ...
                    if (container) {
                        ...
                        container.innerHTML = activePromocodes.map(([code, data]) => {
                            ...
                            const categories = data.applicable_categories ? data.applicable_categories.join(', ') : 'all';
                            return `<div class="promo-card" onclick="copyPromoCode('${code}')">
                                <div class="promo-code">${code}</div>
                                <div class="promo-discount">-${discount}</div>
                                <div class="promo-discount" style="font-size:0.7rem;">👆 Нажмите чтобы скопировать</div>
                            </div>`;
                        }).join('');
                    }
                });
                            ...
                            ${usersArray.map(u => {
                                ...
                                <div class="admin-user-card">
                                    ...
                                    <div class="admin-user-actions">
                                        ${!u.is_admin ? `<button class="edit-btn" onclick="makeAdmin('${u.email}')">Сделать админом</button>` : `<button class="delete-btn" onclick="removeAdmin('${u.email}')">Снять админа</button>`}
