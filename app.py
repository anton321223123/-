    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body { background: linear-gradient(135deg, #0a0a0a 0%, #1a0a0a 100%); display: flex; justify-content: center; align-items: center; min-height: 100vh; font-family: 'Segoe UI', Arial, sans-serif; padding: 20px; }
            .error-container { text-align: center; animation: fadeInUp 0.8s ease-out; max-width: 500px; }
            @keyframes fadeInUp { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }
            @keyframes pulse { 0% { transform: scale(1); } 50% { transform: scale(1.05); } 100% { transform: scale(1); } }
            .triangle { width: 0; height: 0; border-left: 80px solid transparent; border-right: 80px solid transparent; border-bottom: 140px solid #e74c3c; margin: 0 auto 2rem; position: relative; animation: pulse 2s ease-in-out infinite; }
            .triangle::before { content: "!"; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); font-size: 5rem; font-weight: bold; color: white; text-shadow: 2px 2px 4px rgba(0,0,0,0.3); }
            .error-text { font-size: 2rem; font-weight: 500; color: #e74c3c; margin-bottom: 1rem; letter-spacing: 2px; }
            .error-message { color: #888; font-size: 1.1rem; margin-bottom: 2rem; }
            .error-message span { color: #27ae60; font-weight: bold; }
            .logo { margin-top: 2rem; font-size: 1rem; color: #555; letter-spacing: 2px; }
            .logo span { color: #27ae60; font-weight: bold; }
            @media (max-width: 768px) { .triangle { border-left-width: 60px; border-right-width: 60px; border-bottom-width: 105px; } .triangle::before { font-size: 3.5rem; } .error-text { font-size: 1.5rem; } .error-message { font-size: 0.9rem; } }
        </style>
    </head>
    <body>
        <div class="error-container">
            ...
            <div class="error-message">Приносим свои извинения, ведутся технические работы.<br>Скоро мы вернёмся!<br><br>С уважением, команда <span>ZETTA</span></div>
            <div class="logo"><span>⚡ ZETTA</span> — Профессиональная сборка ПК и IT-услуги</div>
        </div>
    </body>
    </html>
    ''', 503)
    ...
    if ban_info:
        ...
        <head>
            ...
            <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                body { background: linear-gradient(135deg, #0a0a0a 0%, #1a0a0a 100%); display: flex; justify-content: center; align-items: center; min-height: 100vh; font-family: 'Segoe UI', Arial, sans-serif; padding: 20px; }
                .ban-container { background: #1a1a1a; border: 1px solid #e74c3c; border-radius: 16px; padding: 2rem; max-width: 500px; width: 100%; text-align: center; animation: fadeInUp 0.6s ease-out; box-shadow: 0 10px 30px rgba(0,0,0,0.5); }
                @keyframes fadeInUp { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }
                @keyframes pulse { 0% { transform: scale(1); } 50% { transform: scale(1.05); } 100% { transform: scale(1); } }
                ...
                .ban-subtitle { color: #888; margin-bottom: 1.5rem; font-size: 0.85rem; }
                .ban-info { background: #0f0f0f; border-radius: 12px; padding: 1.2rem; text-align: left; margin-bottom: 1.5rem; border-left: 4px solid #e74c3c; }
                .ban-info p { margin: 0.5rem 0; color: #bbb; font-size: 0.9rem; }
                .ban-info strong { color: #27ae60; }
                .ban-reason { background: #2a1a1a; padding: 0.8rem; border-radius: 8px; margin: 0.5rem 0; color: #e74c3c; font-weight: bold; font-size: 0.9rem; }
                .ban-message { background: #1a2a1a; padding: 0.8rem; border-radius: 8px; margin: 0.5rem 0; color: #27ae60; font-style: italic; font-size: 0.9rem; }
                ...
                .back-btn { background: #27ae60; color: white; border: none; padding: 0.8rem 1.5rem; border-radius: 8px; cursor: pointer; font-size: 1rem; margin-top: 1rem; transition: all 0.3s ease; width: 100%; max-width: 200px; }
                .back-btn:hover { background: #229954; transform: scale(1.02); }
                @media (max-width: 480px) { .ban-container { padding: 1.5rem; } .ban-title { font-size: 1.3rem; } .ban-icon { font-size: 3rem; } }
            </style>
        </head>
        <body>
            <div class="ban-container">
                ...
                <p style="color: #888; font-size: 0.8rem;">Если вы считаете, что это ошибка, свяжитесь с нами по почте <a href="mailto:zetta_report@zetta22.ru" class="contact-link">zetta_report@zetta22.ru</a></p>
...
def consultation():
    ...
    phones = {'system_admin': '89836074115', 'director': '89520062357', 'manager': '89132447707'}
    names = {'system_admin': 'Системный администратор', 'director': 'Генеральный директор', 'manager': 'Менеджер'}
    if choice in phones:
...
def send_feedback():
    ...
    try:
        ...
        <body style="font-family: Arial, sans-serif;">
            ...
            <div style="background: #f0f0f0; padding: 1rem; border-radius: 8px; margin-top: 0.5rem;">{message.replace(chr(10), '<br>')}</div>
            <hr>
...
def calculate_delivery():
    ...
    return jsonify({'distance': distance, 'delivery_period': period_text, 'delivery_date': delivery_date})
...
def get_cart():
    ...
    return jsonify({'items': items, 'subtotal': subtotal, 'discount': discount, 'total': total})
...
def admin_add_promocode():
    ...
    promocodes[code] = {'discount': discount, 'type': type, 'active': True}
...
# ==================== HTML_TEMPLATE ====================
# ПОЛНЫЙ HTML ТЕМПЛЕЙТ (ВАШ ОРИГИНАЛЬНЫЙ КОД)
...
<head>
    ...
    <style>
        * {
        ...
        /* АНИМАЦИЯ МОЛНИИ - ДОБАВЛЕНА! */
        @keyframes lightningFlash {
            0%, 100% { text-shadow: 0 0 0px #ffd700; transform: scale(1); }
            15% { text-shadow: 0 0 10px #ffd700, 0 0 20px #ffaa00; transform: scale(1.05); color: #fff; }
            30% { text-shadow: 0 0 5px #ffd700; transform: scale(1); }
            45% { text-shadow: 0 0 20px #ffd700, 0 0 35px #ffaa00; transform: scale(1.1); color: #ffffaa; }
            60% { text-shadow: 0 0 8px #ffd700; transform: scale(1); }
            75% { text-shadow: 0 0 15px #ffd700, 0 0 25px #ff6600; transform: scale(1.05); color: #ffffff; }
        }

        @keyframes lightningIcon {
            0% { transform: scale(1); filter: drop-shadow(0 0 0px #ffd700); }
            10% { transform: scale(1.2); filter: drop-shadow(0 0 5px #ffd700); }
            20% { transform: scale(0.9); filter: drop-shadow(0 0 15px #ffaa00); }
            30% { transform: scale(1.3); filter: drop-shadow(0 0 25px #ff6600); }
            40% { transform: scale(1); filter: drop-shadow(0 0 10px #ffd700); }
            50% { transform: scale(1.1); filter: drop-shadow(0 0 20px #ffaa00); }
            60% { transform: scale(0.95); filter: drop-shadow(0 0 5px #ffd700); }
            70% { transform: scale(1.25); filter: drop-shadow(0 0 30px #ff6600); }
            80% { transform: scale(1); filter: drop-shadow(0 0 12px #ffd700); }
            90% { transform: scale(1.05); filter: drop-shadow(0 0 18px #ffaa00); }
        ...
        .logo-icon {
            ...
            animation: lightningIcon 2.5s ease-in-out infinite;
        }

        .logo-icon:hover {
            animation: lightningIcon 0.8s ease-in-out infinite;
        }
        ...
        .team-card, .contact-card, .news-card, .product-card, .home-review-card, .promo-card { animation: fadeInUp 0.6s ease-out backwards; animation-fill-mode: both; }
        ...
        .product-card { animation-delay: calc(var(--index, 0) * 0.05s); }
        ...
        @media (max-width: 768px) {
            ...
            .team-card.center { transform: scale(1) !important; order: -1; }
            .contacts-grid { grid-template-columns: 1fr !important; gap: 1rem !important; }
            .catalog-page-wrapper { flex-direction: column !important; }
            .catalog-sidebar { width: 100% !important; position: static !important; margin-bottom: 1rem; }
            .cart-panel { width: 100% !important; right: -100% !important; }
            .header-content { flex-direction: column !important; padding: 0.75rem 1rem !important; }
            .nav-links { gap: 1rem !important; flex-wrap: wrap; justify-content: center; }
            .nav-link { font-size: 0.8rem !important; }
            .hero h1 { font-size: 1.8rem !important; }
            .hero p { font-size: 0.9rem !important; }
            .hero { padding: 2rem 1rem !important; }
            .search-bar-full { padding: 0.5rem 1rem !important; }
            .search-input-full { font-size: 0.9rem !important; padding: 0.6rem 1rem !important; }
            .search-btn-full { padding: 0.6rem 1rem !important; font-size: 0.8rem !important; }
            .team-avatar { width: 100px !important; height: 100px !important; }
            .team-name { font-size: 1rem !important; }
            .team-position { font-size: 0.75rem !important; }
            .team-description { font-size: 0.75rem !important; }
            .promo-card { padding: 0.5rem !important; }
            .promo-code { font-size: 0.8rem !important; }
            .promo-discount { font-size: 0.7rem !important; }
            .vlog-text { font-size: 0.85rem !important; padding: 1rem !important; }
            .news-card-title { font-size: 0.9rem !important; }
            .news-card-text { font-size: 0.75rem !important; }
            .carousel-slide { flex-direction: column !important; gap: 1rem !important; }
            .home-review-card { padding: 0.75rem !important; }
            .home-review-author { font-size: 0.8rem !important; }
            .home-review-text { font-size: 0.75rem !important; }
            .chat-window { width: 90% !important; right: 5% !important; left: 5% !important; bottom: 80px !important; height: 70vh !important; max-height: 500px; }
            .chat-button { bottom: 15px !important; right: 15px !important; width: 50px !important; height: 50px !important; font-size: 20px !important; }
            .chat-message { font-size: 0.8rem !important; padding: 0.5rem 0.8rem !important; }
            .chat-question-btn { padding: 0.6rem !important; font-size: 0.8rem !important; }
            .footer-content { flex-direction: column !important; text-align: center; gap: 1rem !important; }
            .footer-section { flex-wrap: wrap !important; justify-content: center; gap: 0.8rem !important; }
            .footer-section a, .footer-section span { font-size: 0.75rem !important; white-space: normal !important; }
            .auth-container { width: 95% !important; margin: 10% auto !important; padding: 1.5rem !important; }
            .auth-tab { font-size: 0.9rem !important; padding: 0.4rem 0.8rem !important; }
            .auth-input { font-size: 0.9rem !important; padding: 0.6rem !important; }
            .auth-btn { font-size: 0.9rem !important; padding: 0.6rem !important; }
            .product-title { font-size: 0.85rem !important; }
            .product-price { font-size: 0.9rem !important; }
            .add-to-cart { font-size: 0.75rem !important; padding: 0.4rem !important; }
            .write-review { padding: 1rem !important; }
            .write-review h3 { font-size: 1.1rem !important; }
            .star { font-size: 1.8rem !important; }
            .review-name-input, .review-input { font-size: 0.9rem !important; }
            .submit-review-btn { font-size: 0.9rem !important; padding: 0.7rem !important; }
            .profile-info { padding: 1rem !important; }
            .profile-field { margin-bottom: 0.75rem !important; }
            .profile-label { font-size: 0.7rem !important; }
            .profile-value { font-size: 0.9rem !important; }
            .order-card { padding: 1rem !important; }
            .order-header { flex-direction: column; align-items: flex-start; gap: 0.5rem; }
            .order-number { font-size: 0.9rem !important; }
            .order-status { font-size: 0.7rem !important; }
            .order-date { font-size: 0.7rem !important; }
            .order-item { font-size: 0.8rem !important; }
            .order-total { font-size: 0.9rem !important; }
            .checkout-form { padding: 1rem !important; }
            .form-label { font-size: 0.8rem !important; }
            .form-input { font-size: 0.9rem !important; padding: 0.6rem !important; }
            .payment-option { padding: 0.7rem !important; font-size: 0.8rem !important; }
            .submit-btn { font-size: 0.9rem !important; padding: 0.7rem !important; }
            .admin-form { padding: 1rem !important; }
            .admin-form h3 { font-size: 1rem !important; }
            .admin-tab { font-size: 0.8rem !important; padding: 0.4rem 0.7rem !important; }
            .admin-product-card { flex-direction: column; text-align: center; }
            .admin-user-actions { justify-content: center; }
            .modal-content { width: 95% !important; margin: 10% auto !important; }
            .verify-container, .reset-container, .profile-form-container, .feedback-container { width: 95% !important; margin: 15% auto !important; padding: 1.5rem !important; }
            .verify-code-input { font-size: 1.2rem !important; padding: 0.8rem !important; }
        }
        ...
        @media (max-width: 480px) {
            ...
            .logo-icon { font-size: 1.5rem !important; }
            .team-avatar { width: 80px !important; height: 80px !important; }
            .team-card { padding: 1rem !important; }
            .vlog-edit-btn { font-size: 0.7rem !important; padding: 0.3rem 0.7rem !important; }
            .cart-item { flex-direction: column; gap: 0.5rem; text-align: center; }
            .cart-item-title { font-size: 0.85rem !important; }
            .promo-input-group { flex-direction: column; }
            .apply-promo-btn { width: 100%; }
            .checkout-btn { font-size: 0.8rem !important; }
            .contact-card { padding: 1rem !important; }
            .contact-title { font-size: 0.9rem !important; }
            .contact-value { font-size: 0.8rem !important; }
            .map-placeholder { padding: 1rem !important; }
        }

        /* Основные стили */
        .header { background: #0a0a0a; border-bottom: 1px solid #2a2a2a; position: sticky; top: 0; z-index: 100; }
        .header-content { max-width: 1280px; margin: 0 auto; padding: 1rem 2rem; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; }
        .logo { display: flex; align-items: center; gap: 0.5rem; cursor: pointer; transition: all 0.3s ease; }
        .logo:hover { opacity: 0.8; transform: scale(1.02); }
        .nav-links { display: flex; gap: 2rem; align-items: center; flex-wrap: wrap; }
        .nav-link { cursor: pointer; transition: all 0.3s ease; font-size: 0.9rem; text-transform: uppercase; letter-spacing: 1px; position: relative; }
        .nav-link::after { content: ''; position: absolute; bottom: -5px; left: 0; width: 0; height: 2px; background: #27ae60; transition: width 0.3s ease; }
        .nav-link:hover::after { width: 100%; }
        .nav-link:hover { color: #27ae60; }
        .nav-link.active { color: #27ae60; }
        .nav-link.active::after { width: 100%; }
        .search-bar-full { background: #0f0f0f; border-top: 1px solid #2a2a2a; border-bottom: 1px solid #2a2a2a; padding: 0.75rem 2rem; width: 100%; }
        .search-container { max-width: 1280px; margin: 0 auto; display: flex; gap: 0.5rem; }
        .search-input-full { flex: 1; padding: 0.75rem 1.25rem; background: #1a1a1a; border: 1px solid #3a3a3a; color: #e0e0e0; font-size: 1rem; border-radius: 8px; transition: all 0.3s ease; }
        .search-input-full:focus { outline: none; border-color: #27ae60; box-shadow: 0 0 10px rgba(39, 174, 96, 0.3); }
        .search-btn-full { padding: 0.75rem 1.5rem; background: #27ae60; color: white; border: none; cursor: pointer; font-size: 0.9rem; border-radius: 8px; transition: all 0.3s ease; font-weight: 500; }
        .search-btn-full:hover { background: #229954; transform: scale(1.02); }
        .user-icon { position: relative; cursor: pointer; font-size: 1.25rem; padding: 0.5rem; display: flex; align-items: center; gap: 0.5rem; transition: all 0.3s ease; border-radius: 8px; }
        .user-icon:hover { background: #2a2a2a; transform: scale(1.05); }
        .user-name { font-size: 0.9rem; color: #27ae60; }
        .cart-icon { position: relative; cursor: pointer; font-size: 1.25rem; padding: 0.5rem; transition: all 0.3s ease; border-radius: 8px; }
        .cart-icon:hover { transform: scale(1.1); background: #2a2a2a; }
        .cart-count { position: absolute; top: 0; right: 0; background: #e74c3c; color: white; font-size: 0.7rem; padding: 0.15rem 0.4rem; border-radius: 2px; }
        .container { max-width: 1280px; margin: 0 auto; padding: 2rem; flex: 1; }
        .catalog-page-wrapper { display: flex; gap: 2rem; }
        .catalog-sidebar { width: 260px; flex-shrink: 0; background: #0f0f0f; border: 1px solid #2a2a2a; border-radius: 12px; padding: 1.5rem; height: fit-content; position: sticky; top: 90px; }
        .catalog-sidebar h3 { color: #27ae60; font-size: 1rem; margin-bottom: 1.5rem; text-transform: uppercase; letter-spacing: 1px; }
        .category-list { display: flex; flex-direction: column; gap: 0.75rem; }
        .category-item { display: flex; align-items: center; gap: 0.75rem; padding: 0.6rem 0.8rem; cursor: pointer; transition: all 0.3s ease; border-radius: 8px; color: #888; }
        .category-item:hover { background: #1a1a1a; color: #27ae60; transform: translateX(5px); }
        .category-item.active { background: #1a2a1a; color: #27ae60; border-left: 3px solid #27ae60; }
        .category-icon { font-size: 1.2rem; }
        .category-name { font-size: 0.9rem; }
        .main-content { flex: 1; min-width: 0; }

        .admin-panel { background: #0f0f0f; border: 1px solid #2a2a2a; padding: 1.5rem; margin-bottom: 2rem; border-radius: 12px; }
        .admin-tabs { display: flex; gap: 1rem; border-bottom: 1px solid #2a2a2a; margin-bottom: 1.5rem; flex-wrap: wrap; }
        .admin-tab { padding: 0.5rem 1rem; cursor: pointer; color: #888; transition: all 0.3s ease; }
        .admin-tab:hover { color: #27ae60; }
        .admin-tab.active { color: #27ae60; border-bottom: 2px solid #27ae60; }
        .admin-section { display: none; animation: fadeInUp 0.4s ease-out; }
        .admin-section.active { display: block; }
        .admin-form { background: #1a1a1a; padding: 1.5rem; margin-bottom: 1.5rem; border-radius: 12px; transition: all 0.3s ease; }
        .admin-form:hover { transform: translateY(-2px); box-shadow: 0 5px 20px rgba(0,0,0,0.3); }
        .admin-form input, .admin-form textarea, .admin-form select { width: 100%; padding: 0.75rem; margin-bottom: 1rem; background: #2a2a2a; border: 1px solid #3a3a3a; color: #e0e0e0; border-radius: 8px; transition: all 0.3s ease; }
        .admin-form input:focus, .admin-form textarea:focus, .admin-form select:focus { border-color: #27ae60; outline: none; box-shadow: 0 0 10px rgba(39, 174, 96, 0.3); }
        .admin-form button { padding: 0.75rem 1.5rem; background: #27ae60; color: white; border: none; cursor: pointer; border-radius: 8px; transition: all 0.3s ease; }
        .admin-form button:hover { background: #229954; transform: scale(1.02); }
        .admin-products-grid { display: flex; flex-direction: column; gap: 1rem; }
        .admin-product-card { background: #2a2a2a; border: 1px solid #3a3a3a; border-radius: 8px; padding: 1rem; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; transition: all 0.3s ease; }
        .admin-product-card:hover { transform: translateX(5px); border-color: #27ae60; }
        .admin-product-info { flex: 1; min-width: 200px; }
        .admin-product-name { font-weight: bold; font-size: 1rem; color: #27ae60; }
        .admin-product-price { font-size: 0.85rem; color: #888; margin-top: 0.25rem; }
        .admin-product-actions { display: flex; gap: 0.5rem; }
        .admin-product-actions button { padding: 0.5rem 1rem; margin: 0; }
        .admin-news-list { display: flex; flex-direction: column; gap: 1rem; }
        .admin-news-card { background: #2a2a2a; border: 1px solid #3a3a3a; border-radius: 8px; padding: 1rem; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; transition: all 0.3s ease; }
        .admin-news-card:hover { transform: translateX(5px); border-color: #27ae60; }
        .admin-news-info { flex: 1; }
        .admin-news-title { font-weight: bold; font-size: 1rem; color: #27ae60; }
        .admin-news-date { font-size: 0.8rem; color: #888; margin-top: 0.25rem; }
        .admin-news-actions { display: flex; gap: 0.5rem; }
        .admin-users-list { display: flex; flex-direction: column; gap: 1rem; }
        .admin-user-card { background: #2a2a2a; border: 1px solid #3a3a3a; border-radius: 8px; padding: 1rem; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; transition: all 0.3s ease; }
        .admin-user-card:hover { transform: translateX(5px); border-color: #27ae60; }
        .admin-user-info { flex: 1; }
        .admin-user-email { font-weight: bold; font-size: 1rem; color: #27ae60; }
        .admin-user-details { font-size: 0.8rem; color: #888; margin-top: 0.25rem; }
        .admin-user-actions { display: flex; gap: 0.5rem; flex-wrap: wrap; }
        .ban-select { padding: 0.3rem; background: #2a2a2a; border: 1px solid #3a3a3a; color: #e0e0e0; border-radius: 3px; }
        .ban-reason-input, .ban-message-input { width: 200px; padding: 0.3rem; background: #2a2a2a; border: 1px solid #3a3a3a; color: #e0e0e0; border-radius: 3px; font-size: 0.8rem; }
        .admin-promocodes-list { display: flex; flex-direction: column; gap: 1rem; }
        .admin-promocode-card { background: #2a2a2a; border: 1px solid #3a3a3a; border-radius: 8px; padding: 1rem; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; transition: all 0.3s ease; }
        .admin-promocode-card:hover { transform: translateX(5px); border-color: #27ae60; }
        .admin-promocode-info { flex: 1; }
        .admin-promocode-code { font-weight: bold; font-size: 1rem; color: #27ae60; font-family: monospace; }
        .admin-promocode-details { font-size: 0.8rem; color: #888; margin-top: 0.25rem; }
        .admin-promocode-status { display: inline-block; padding: 0.2rem 0.5rem; border-radius: 3px; font-size: 0.7rem; margin-left: 0.5rem; }
        .admin-promocode-status.active { background: #27ae60; color: white; }
        .admin-promocode-status.inactive { background: #e74c3c; color: white; }
        .admin-table { width: 100%; border-collapse: collapse; }
        .admin-table th, .admin-table td { padding: 0.75rem; text-align: left; border-bottom: 1px solid #2a2a2a; }
        .admin-table th { background: #1a1a1a; color: #27ae60; }
        .delete-btn, .edit-btn, .toggle-btn, .ban-btn, .unban-btn { padding: 0.25rem 0.5rem; cursor: pointer; border: none; margin: 0 0.25rem; border-radius: 4px; transition: all 0.3s ease; }
        .delete-btn:hover, .edit-btn:hover, .toggle-btn:hover, .ban-btn:hover, .unban-btn:hover { transform: scale(1.05); opacity: 0.9; }
        ...
        .hero { padding: 4rem 2rem; border-bottom: 1px solid #2a2a2a; margin-bottom: 2rem; text-align: center; background: linear-gradient(135deg, #0a0a0a 0%, #1a2a1a 100%); border-radius: 12px; }
        .hero h1 { font-size: 2.5rem; font-weight: 500; margin-bottom: 1rem; background: linear-gradient(135deg, #fff, #27ae60); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
        .hero p { color: #888; font-size: 1.1rem; }
        .team-section { background: #0f0f0f; border: 1px solid #2a2a2a; padding: 1.5rem; margin-bottom: 2rem; border-radius: 12px; }
        .team-title { font-size: 1rem; font-weight: 500; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 1.5rem; text-align: center; color: #e0e0e0; }
        .team-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 2rem; text-align: center; }
        .team-card { background: #1a1a1a; border: 1px solid #2a2a2a; padding: 1.5rem; transition: all 0.3s ease; border-radius: 12px; }
        .team-card:hover { transform: translateY(-5px); border-color: #27ae60; }
        .team-card.center { transform: scale(1.02); border-color: #27ae60; }
        .team-avatar { width: 150px; height: 150px; border-radius: 50%; margin: 0 auto 1rem; background-size: cover; background-position: center; border: 3px solid #2a2a2a; transition: all 0.3s ease; }
        .team-card:hover .team-avatar { transform: scale(1.05); }
        .team-card.center .team-avatar { border-color: #27ae60; }
        .team-name { font-size: 1.2rem; font-weight: 500; margin-bottom: 0.25rem; color: #e0e0e0; }
        .team-position { font-size: 0.85rem; color: #27ae60; margin-bottom: 0.75rem; }
        .team-description { font-size: 0.85rem; color: #888; line-height: 1.4; }
        .news-carousel-section { background: #0f0f0f; border: 1px solid #2a2a2a; padding: 1.5rem; margin-bottom: 2rem; position: relative; border-radius: 12px; }
        .admin-add-btn { position: absolute; top: 1rem; right: 1rem; background: #27ae60; color: white; border: none; width: 36px; height: 36px; border-radius: 50%; cursor: pointer; font-size: 1.5rem; display: flex; align-items: center; justify-content: center; transition: all 0.3s ease; }
        .admin-add-btn:hover { transform: scale(1.1); background: #229954; }
        ...
        .carousel-slide { min-width: 100%; display: flex; gap: 1.5rem; padding: 1rem; }
        .news-card { flex: 1; background: #1a1a1a; border: 1px solid #2a2a2a; border-radius: 12px; overflow: hidden; transition: all 0.3s ease; cursor: pointer; position: relative; }
        .news-card:hover { transform: translateY(-5px); border-color: #27ae60; box-shadow: 0 10px 25px rgba(0,0,0,0.3); }
        .news-card-image { width: 100%; height: 200px; object-fit: cover; transition: transform 0.3s ease; }
        .news-card:hover .news-card-image { transform: scale(1.05); }
        .news-card-content { padding: 1rem; }
        .news-card-date { font-size: 0.7rem; color: #27ae60; margin-bottom: 0.5rem; }
        .news-card-title { font-size: 1rem; font-weight: 500; margin-bottom: 0.5rem; color: #e0e0e0; }
        .news-card-text { font-size: 0.85rem; color: #888; line-height: 1.4; }
        .carousel-btn { position: absolute; top: 50%; transform: translateY(-50%); background: rgba(0,0,0,0.7); color: white; border: none; width: 40px; height: 40px; border-radius: 50%; cursor: pointer; font-size: 1.5rem; z-index: 10; transition: all 0.3s ease; }
        .carousel-btn:hover { background: #27ae60; transform: translateY(-50%) scale(1.1); }
        .carousel-btn.prev { left: 10px; }
        .carousel-btn.next { right: 10px; }
        .carousel-dots { display: flex; justify-content: center; gap: 0.5rem; margin-top: 1rem; }
        .dot { width: 10px; height: 10px; border-radius: 50%; background: #3a3a3a; cursor: pointer; transition: all 0.3s ease; }
        .dot:hover { background: #27ae60; }
        .dot.active { background: #27ae60; width: 20px; border-radius: 5px; }
        .news-modal { display: none; position: fixed; z-index: 2000; left: 0; top: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.95); }
        .news-modal-content { background: #0f0f0f; margin: 5% auto; width: 90%; max-width: 800px; border: 1px solid #2a2a2a; border-radius: 12px; overflow: hidden; animation: scaleIn 0.4s ease-out; }
        .news-modal-header { padding: 1.25rem; border-bottom: 1px solid #2a2a2a; display: flex; justify-content: space-between; align-items: center; }
        .news-modal-header h2 { font-size: 1.3rem; color: #e0e0e0; }
        .news-modal-body { padding: 1.5rem; }
        .news-modal-image { width: 100%; max-height: 400px; object-fit: cover; border-radius: 8px; margin-bottom: 1rem; }
        .news-modal-date { color: #27ae60; font-size: 0.8rem; margin-bottom: 1rem; }
        .news-modal-fulltext { color: #bbb; line-height: 1.6; font-size: 1rem; }
        .close-news-modal { font-size: 1.5rem; cursor: pointer; color: #666; transition: all 0.3s ease; }
        .close-news-modal:hover { color: #fff; transform: scale(1.1); }
        .verify-modal { display: none; position: fixed; z-index: 3000; left: 0; top: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.95); }
        .verify-container { background: #0f0f0f; margin: 10% auto; width: 90%; max-width: 450px; border: 1px solid #2a2a2a; padding: 2rem; text-align: center; border-radius: 12px; animation: scaleIn 0.4s ease-out; }
        .verify-code-input { width: 100%; padding: 1rem; font-size: 1.5rem; text-align: center; letter-spacing: 5px; background: #1a1a1a; border: 1px solid #3a3a3a; color: #e0e0e0; margin: 1rem 0; font-family: monospace; border-radius: 8px; }
        .verify-timer { color: #27ae60; font-size: 0.9rem; margin-bottom: 1rem; }
        .resend-code-btn { background: transparent; border: 1px solid #3a3a3a; color: #e0e0e0; padding: 0.5rem 1rem; cursor: pointer; margin-top: 0.5rem; border-radius: 8px; transition: all 0.3s ease; }
        .resend-code-btn:hover { border-color: #27ae60; color: #27ae60; transform: scale(1.02); }
        .reset-modal { display: none; position: fixed; z-index: 3000; left: 0; top: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.95); }
        .reset-container { background: #0f0f0f; margin: 10% auto; width: 90%; max-width: 450px; border: 1px solid #2a2a2a; padding: 2rem; text-align: center; border-radius: 12px; animation: scaleIn 0.4s ease-out; }
        .profile-form-modal { display: none; position: fixed; z-index: 4000; left: 0; top: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.95); }
        .profile-form-container { background: #0f0f0f; margin: 10% auto; width: 90%; max-width: 450px; border: 1px solid #2a2a2a; padding: 2rem; border-radius: 12px; animation: scaleIn 0.4s ease-out; }
        .contacts-page { max-width: 100%; margin: 0 auto; }
        .contacts-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1.5rem; margin-bottom: 2rem; }
        .contact-card { background: #0f0f0f; border: 1px solid #2a2a2a; padding: 1.5rem; text-align: center; transition: all 0.3s ease; border-radius: 12px; }
        .contact-card:hover { border-color: #27ae60; transform: translateY(-5px); box-shadow: 0 10px 25px rgba(0,0,0,0.3); }
        .contact-icon { font-size: 2rem; margin-bottom: 1rem; transition: all 0.3s ease; }
        .contact-card:hover .contact-icon { transform: scale(1.1); }
        .contact-title { font-size: 1rem; font-weight: 500; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 1rem; color: #27ae60; }
        .contact-value { color: #e0e0e0; line-height: 1.6; }
        .map-placeholder { background: #0f0f0f; border: 1px solid #2a2a2a; padding: 2rem; text-align: center; border-radius: 12px; transition: all 0.3s ease; }
        .map-placeholder:hover { border-color: #27ae60; }
        .reviews-section { background: #0f0f0f; border: 1px solid #2a2a2a; padding: 1.5rem; margin-bottom: 2rem; border-radius: 12px; }
        .reviews-title { font-size: 1rem; font-weight: 500; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 1.5rem; color: #e0e0e0; }
        .home-reviews-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1.5rem; }
        .home-review-card { background: #1a1a1a; border: 1px solid #2a2a2a; padding: 1rem; transition: all 0.3s ease; border-radius: 8px; }
        .home-review-card:hover { border-color: #27ae60; transform: translateY(-3px); box-shadow: 0 5px 15px rgba(0,0,0,0.2); }
        .home-review-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.75rem; flex-wrap: wrap; gap: 0.5rem; }
        .home-review-author { font-weight: 500; color: #27ae60; font-size: 0.9rem; }
        .home-review-date { font-size: 0.7rem; color: #666; }
        .home-review-stars { display: flex; gap: 0.2rem; margin-bottom: 0.75rem; }
        .home-review-stars .star-static { font-size: 0.9rem; color: #555; }
        .home-review-stars .star-static.active { color: #ffc107; }
        .home-review-text { color: #bbb; font-size: 0.85rem; line-height: 1.5; }
        .vlog-section { background: #0f0f0f; border: 1px solid #2a2a2a; padding: 1.5rem; margin-bottom: 2rem; border-radius: 12px; }
        .vlog-content { display: flex; flex-direction: column; gap: 1rem; }
        .vlog-text { background: #1a1a1a; padding: 1.5rem; border-radius: 12px; line-height: 1.6; color: #bbb; font-size: 1rem; border-left: 3px solid #27ae60; white-space: pre-wrap; }
        .vlog-edit-btn { align-self: flex-end; padding: 0.5rem 1rem; background: #27ae60; color: white; border: none; border-radius: 8px; cursor: pointer; transition: all 0.3s ease; }
        .vlog-edit-btn:hover { background: #229954; transform: scale(1.02); }
        .feedback-modal { display: none; position: fixed; z-index: 5000; left: 0; top: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.95); }
        .feedback-container { background: #0f0f0f; margin: 10% auto; width: 90%; max-width: 500px; border: 1px solid #2a2a2a; padding: 2rem; border-radius: 12px; position: relative; animation: scaleIn 0.4s ease-out; }
        .feedback-container h3 { color: #27ae60; margin-bottom: 1rem; }
        .feedback-container p { color: #888; margin-bottom: 1rem; font-size: 0.9rem; }
        .feedback-textarea { width: 100%; padding: 1rem; background: #1a1a1a; border: 1px solid #3a3a3a; color: #e0e0e0; border-radius: 8px; resize: vertical; font-family: inherit; }
        .feedback-textarea:focus { outline: none; border-color: #27ae60; }
        .feedback-send-btn { flex: 1; padding: 0.75rem; background: #27ae60; color: white; border: none; border-radius: 8px; cursor: pointer; transition: all 0.3s ease; }
        .feedback-send-btn:hover { background: #229954; transform: scale(1.02); }
        .feedback-cancel-btn { flex: 1; padding: 0.75rem; background: #e74c3c; color: white; border: none; border-radius: 8px; cursor: pointer; transition: all 0.3s ease; }
        .feedback-cancel-btn:hover { background: #c0392b; transform: scale(1.02); }
        .close-feedback { position: absolute; top: 1rem; right: 1rem; font-size: 1.5rem; cursor: pointer; color: #666; transition: all 0.3s ease; }
        .close-feedback:hover { color: #fff; transform: scale(1.1); }
        .promo-section { background: #0f0f0f; border: 1px solid #2a2a2a; padding: 1.5rem; margin-bottom: 2rem; border-radius: 12px; }
        .promo-title { font-size: 1rem; font-weight: 500; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 1rem; }
        .promo-codes { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 0.75rem; }
        .promo-card { background: #1a1a1a; border: 1px solid #2a2a2a; padding: 0.75rem; text-align: center; cursor: pointer; transition: all 0.3s ease; border-radius: 8px; }
        .promo-card:hover { border-color: #27ae60; transform: translateY(-2px); animation: glowGreen 0.5s ease; }
        .promo-code { font-family: monospace; font-size: 1rem; font-weight: 500; }
        .promo-discount { font-size: 0.8rem; color: #27ae60; }
        .page-header { margin-bottom: 2rem; padding-bottom: 1rem; border-bottom: 1px solid #2a2a2a; }
        .page-header h1 { font-size: 1.5rem; font-weight: 500; text-transform: uppercase; letter-spacing: 1px; }
        .products-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1.5rem; }
        .product-card { background: #0f0f0f; border: 1px solid #2a2a2a; cursor: pointer; transition: all 0.3s ease; position: relative; overflow: hidden; border-radius: 12px; }
        .product-card:hover { border-color: #27ae60; transform: translateY(-5px); box-shadow: 0 10px 25px rgba(0,0,0,0.3); }
        .product-card:hover .product-image { transform: scale(1.05); }
        .product-card.super-sale { border: 2px solid #e74c3c; background: linear-gradient(135deg, #0f0f0f 0%, #1a0a0a 100%); position: relative; overflow: hidden; }
        .product-card.super-sale::before { content: "🔥 SALE 🔥"; position: absolute; top: 10px; right: -30px; background: #e74c3c; color: white; padding: 5px 30px; transform: rotate(45deg); font-size: 12px; font-weight: bold; z-index: 1; animation: shine 2s infinite linear; background: linear-gradient(90deg, #e74c3c, #ff6b6b, #e74c3c); background-size: 200% auto; }
        .discount-badge { position: absolute; top: 10px; left: 10px; background: #e74c3c; color: white; padding: 5px 10px; border-radius: 5px; font-size: 12px; font-weight: bold; z-index: 2; animation: pulse 1s infinite; }
        ...
        .product-price { font-size: 1rem; font-weight: 500; margin: 0.5rem 0; }
        .product-price.super-price { color: #e74c3c; font-size: 1.3rem; font-weight: bold; }
        .old-price { text-decoration: line-through; color: #666; font-size: 0.8rem; margin-left: 0.5rem; }
        .sale-price { color: #e74c3c; font-weight: bold; }
        .add-to-cart { width: 100%; padding: 0.5rem; background: transparent; color: #e0e0e0; border: 1px solid #3a3a3a; cursor: pointer; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 1px; transition: all 0.3s ease; border-radius: 6px; }
        .add-to-cart:hover { background: #27ae60; border-color: #27ae60; transform: scale(1.02); }
        .reviews-container { width: 100%; margin: 0; padding: 0; }
        .write-review { background: #0f0f0f; border: 1px solid #2a2a2a; padding: 2rem; margin-bottom: 2rem; width: 100%; border-radius: 12px; transition: all 0.3s ease; }
        .write-review:hover { border-color: #27ae60; }
        .write-review h3 { font-size: 1.3rem; margin-bottom: 1rem; color: #27ae60; }
        .stars-rating { display: flex; gap: 0.5rem; margin: 1rem 0; direction: row; justify-content: center; }
        .star { font-size: 2.5rem; cursor: pointer; color: #555; transition: all 0.2s; }
        .star.active { color: #ffc107; }
        .star:hover { color: #ffc107; transform: scale(1.1); }
        .review-name-input { width: 100%; padding: 1rem; background: #1a1a1a; border: 1px solid #3a3a3a; color: #e0e0e0; font-size: 1rem; margin-bottom: 1rem; border-radius: 8px; transition: all 0.3s ease; }
        .review-name-input:focus { border-color: #27ae60; outline: none; box-shadow: 0 0 10px rgba(39, 174, 96, 0.3); }
        .review-input { width: 100%; padding: 1rem; background: #1a1a1a; border: 1px solid #3a3a3a; color: #e0e0e0; font-size: 1rem; resize: vertical; font-family: inherit; border-radius: 8px; min-height: 120px; transition: all 0.3s ease; }
        .review-input:focus { border-color: #27ae60; outline: none; box-shadow: 0 0 10px rgba(39, 174, 96, 0.3); }
        .submit-review-btn { padding: 1rem 2rem; background: #27ae60; color: white; border: none; cursor: pointer; font-size: 1rem; margin-top: 1rem; border-radius: 8px; width: 100%; font-weight: bold; transition: all 0.3s ease; }
        .submit-review-btn:hover { background: #229954; transform: scale(1.02); }
        .reviews-list { background: #0f0f0f; border: 1px solid #2a2a2a; padding: 2rem; width: 100%; border-radius: 12px; }
        .review-item { padding: 1.5rem; border-bottom: 1px solid #2a2a2a; margin-bottom: 1rem; position: relative; background: #1a1a1a; border-radius: 8px; width: 100%; transition: all 0.3s ease; }
        .review-item:hover { transform: translateX(5px); border-left: 3px solid #27ae60; }
        .review-item:last-child { border-bottom: none; margin-bottom: 0; }
        .review-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.75rem; flex-wrap: wrap; gap: 0.5rem; }
        .review-author { font-weight: 600; color: #27ae60; font-size: 1.1rem; }
        .review-date { font-size: 0.85rem; color: #666; }
        .review-stars { display: flex; gap: 0.25rem; margin-bottom: 0.75rem; }
        .review-stars .star-static { font-size: 1.2rem; color: #555; }
        .review-stars .star-static.active { color: #ffc107; }
        .review-text { color: #bbb; line-height: 1.6; font-size: 1rem; }
        .profile-container { max-width: 1000px; margin: 0 auto; }
        .profile-info { background: #0f0f0f; border: 1px solid #2a2a2a; padding: 2rem; margin-bottom: 2rem; border-radius: 12px; transition: all 0.3s ease; }
        .profile-info:hover { border-color: #27ae60; }
        .profile-field { margin-bottom: 1rem; padding-bottom: 1rem; border-bottom: 1px solid #2a2a2a; }
        .profile-label { color: #888; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 1px; }
        .profile-value { font-size: 1.1rem; margin-top: 0.25rem; color: #e0e0e0; }
        .orders-list { background: #0f0f0f; border: 1px solid #2a2a2a; padding: 1.5rem; border-radius: 12px; }
        .order-card { background: #1a1a1a; border: 1px solid #2a2a2a; padding: 1.5rem; margin-bottom: 1rem; transition: all 0.3s ease; border-radius: 8px; }
        .order-card:hover { border-color: #27ae60; transform: translateX(5px); }
        .order-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; flex-wrap: wrap; gap: 0.5rem; padding-bottom: 0.5rem; border-bottom: 1px solid #2a2a2a; }
        .order-number { font-weight: bold; color: #27ae60; }
        .order-status { background: #27ae60; padding: 0.25rem 0.5rem; font-size: 0.8rem; border-radius: 4px; }
        .order-date { color: #888; font-size: 0.8rem; }
        .order-items { margin: 1rem 0; }
        .order-item { display: flex; justify-content: space-between; padding: 0.5rem 0; border-bottom: 1px solid #2a2a2a; }
        .order-total { text-align: right; margin-top: 1rem; padding-top: 0.5rem; font-size: 1.1rem; font-weight: bold; }
        .logout-btn, .profile-edit-btn { padding: 0.5rem 1rem; background: transparent; border: 1px solid #e74c3c; color: #e74c3c; cursor: pointer; margin-top: 1rem; transition: all 0.3s ease; border-radius: 8px; }
        .logout-btn:hover, .profile-edit-btn:hover { transform: scale(1.05); }
        .profile-edit-btn { border-color: #27ae60; color: #27ae60; margin-right: 1rem; }
        .checkout-page { max-width: 800px; margin: 0 auto; }
        .checkout-form { background: #0f0f0f; border: 1px solid #2a2a2a; padding: 2rem; border-radius: 12px; }
        .form-group { margin-bottom: 1.5rem; }
        .form-label { display: block; margin-bottom: 0.5rem; font-size: 0.9rem; color: #888; }
        .form-input { width: 100%; padding: 0.75rem; background: #1a1a1a; border: 1px solid #3a3a3a; color: #e0e0e0; font-size: 1rem; transition: all 0.3s ease; border-radius: 8px; }
        .form-input:focus { outline: none; border-color: #27ae60; box-shadow: 0 0 10px rgba(39, 174, 96, 0.3); }
        .payment-methods { display: flex; gap: 1rem; margin-top: 0.5rem; }
        .payment-option { flex: 1; padding: 1rem; background: #1a1a1a; border: 1px solid #3a3a3a; cursor: pointer; text-align: center; transition: all 0.3s ease; border-radius: 8px; }
        .payment-option:hover { transform: scale(1.02); border-color: #27ae60; }
        .payment-option.selected { border-color: #27ae60; background: #1a2a1a; }
        .card-fields { margin-top: 1rem; padding: 1rem; background: #1a1a1a; border: 1px solid #3a3a3a; border-radius: 8px; animation: fadeInUp 0.3s ease-out; }
        .delivery-info { background: #1a1a1a; padding: 1rem; margin: 1rem 0; border-left: 3px solid #27ae60; border-radius: 8px; }
        .submit-btn { width: 100%; padding: 1rem; background: #27ae60; color: white; border: none; cursor: pointer; font-size: 1rem; text-transform: uppercase; letter-spacing: 1px; margin-top: 1rem; transition: all 0.3s ease; border-radius: 8px; }
        .submit-btn:hover { background: #229954; transform: scale(1.02); }
        .order-summary { background: #1a1a1a; padding: 1rem; margin-bottom: 1rem; border-radius: 8px; }
        .modal { display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.9); }
        .modal-content { background: #0f0f0f; margin: 5% auto; width: 90%; max-width: 700px; border: 1px solid #2a2a2a; border-radius: 12px; animation: scaleIn 0.4s ease-out; }
        .modal-header { padding: 1.25rem; border-bottom: 1px solid #2a2a2a; display: flex; justify-content: space-between; }
        .close { font-size: 1.5rem; cursor: pointer; color: #666; transition: all 0.3s ease; }
        .close:hover { color: #fff; transform: scale(1.1); }
        .cart-panel { position: fixed; right: -450px; top: 0; width: 450px; height: 100%; background: #0a0a0a; z-index: 1001; transition: right 0.3s ease; display: flex; flex-direction: column; border-left: 1px solid #2a2a2a; }
        .cart-panel.open { right: 0; }
        .cart-header { padding: 1.25rem; background: #0f0f0f; border-bottom: 1px solid #2a2a2a; display: flex; justify-content: space-between; }
        .cart-items { flex: 1; overflow-y: auto; padding: 1rem; }
        .cart-item { display: flex; justify-content: space-between; align-items: center; padding: 1rem; border-bottom: 1px solid #2a2a2a; animation: fadeInRight 0.3s ease-out; }
        .cart-item button { transition: all 0.2s ease; }
        .cart-item button:hover { opacity: 0.8; transform: scale(1.05); }
        .cart-item button:active { transform: scale(0.95); }
        .empty-cart { text-align: center; color: #888; padding: 2rem; }
        .cart-item-title { font-weight: 500; margin-bottom: 0.25rem; }
        .cart-footer { padding: 1.25rem; border-top: 1px solid #2a2a2a; }
        .promo-input-group { display: flex; gap: 0.5rem; margin-bottom: 1rem; }
        .promo-input { flex: 1; padding: 0.5rem; background: #1a1a1a; border: 1px solid #3a3a3a; color: #e0e0e0; border-radius: 6px; }
        .apply-promo-btn, .checkout-btn { padding: 0.5rem 1rem; background: transparent; border: 1px solid #3a3a3a; color: #e0e0e0; cursor: pointer; transition: all 0.3s ease; border-radius: 6px; }
        .apply-promo-btn:hover, .checkout-btn:hover { background: #27ae60; border-color: #27ae60; transform: scale(1.02); }
        .checkout-btn { width: 100%; border-color: #27ae60; color: #27ae60; margin-top: 0.5rem; }
        .overlay { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); z-index: 1000; transition: all 0.3s ease; }
        .hidden { display: none; }
        .auth-modal { display: none; position: fixed; z-index: 2000; left: 0; top: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.95); }
        .auth-container { background: #0f0f0f; margin: 5% auto; width: 90%; max-width: 450px; border: 1px solid #2a2a2a; padding: 2rem; border-radius: 12px; animation: scaleIn 0.4s ease-out; }
        .auth-tabs { display: flex; gap: 1rem; margin-bottom: 2rem; border-bottom: 1px solid #2a2a2a; }
        .auth-tab { padding: 0.5rem 1rem; cursor: pointer; color: #888; transition: all 0.3s ease; }
        .auth-tab:hover { color: #27ae60; }
        .auth-tab.active { color: #27ae60; border-bottom: 2px solid #27ae60; }
        .auth-form { display: flex; flex-direction: column; gap: 1rem; }
        .auth-form.hidden { display: none; }
        .auth-input { padding: 0.75rem; background: #1a1a1a; border: 1px solid #3a3a3a; color: #e0e0e0; transition: all 0.3s ease; border-radius: 8px; }
        .auth-input:focus { border-color: #27ae60; outline: none; box-shadow: 0 0 10px rgba(39, 174, 96, 0.3); }
        .auth-btn { padding: 0.75rem; background: #27ae60; color: white; border: none; cursor: pointer; margin-top: 1rem; transition: all 0.3s ease; border-radius: 8px; }
        .auth-btn:hover { background: #229954; transform: scale(1.02); }
        .forgot-password { text-align: right; margin-top: -0.5rem; }
        .forgot-password a { color: #27ae60; font-size: 0.8rem; cursor: pointer; text-decoration: none; transition: all 0.3s ease; }
        .forgot-password a:hover { text-decoration: underline; color: #229954; }
        .chat-button { position: fixed; bottom: 20px; right: 20px; width: 60px; height: 60px; border-radius: 50%; background: #27ae60; color: white; border: none; cursor: pointer; font-size: 24px; z-index: 10000; box-shadow: 0 4px 15px rgba(0,0,0,0.3); transition: all 0.3s ease; display: flex; align-items: center; justify-content: center; }
        .chat-button:hover { transform: scale(1.1); background: #229954; animation: glowGreen 1s infinite; }
        .chat-window { position: fixed; bottom: 95px; right: 20px; width: 380px; height: 500px; background: #0f0f0f; border: 1px solid #2a2a2a; border-radius: 12px; display: none; flex-direction: column; z-index: 10001; box-shadow: 0 5px 25px rgba(0,0,0,0.5); overflow: hidden; animation: scaleIn 0.3s ease-out; }
        .chat-window.open { display: flex; }
        .chat-header { padding: 1rem; background: #1a1a1a; border-bottom: 1px solid #2a2a2a; display: flex; justify-content: space-between; align-items: center; flex-shrink: 0; }
        .chat-header h3 { color: #27ae60; font-size: 1rem; margin: 0; }
        .close-chat { cursor: pointer; font-size: 1.2rem; color: #888; transition: all 0.2s; }
        .close-chat:hover { color: #e74c3c; transform: scale(1.1); }
        .chat-messages-area { flex: 1; overflow-y: auto; padding: 1rem; display: flex; flex-direction: column; gap: 0.5rem; min-height: 0; }
        .chat-message { max-width: 85%; padding: 0.6rem 1rem; border-radius: 12px; word-wrap: break-word; line-height: 1.4; font-size: 0.9rem; animation: fadeInUp 0.3s ease; }
        .chat-message.user { background: #27ae60; color: white; align-self: flex-end; border-bottom-right-radius: 4px; }
        .chat-message.admin { background: #e74c3c; color: white; align-self: flex-start; border-bottom-left-radius: 4px; }
        .chat-message.system { background: #c0392b; color: white; align-self: center; font-size: 0.75rem; text-align: center; border-radius: 15px; max-width: 90%; }
        .chat-buttons { padding: 1rem; border-top: 1px solid #2a2a2a; display: flex; flex-direction: column; gap: 0.5rem; background: #0f0f0f; flex-shrink: 0; }
        .chat-question-btn { padding: 0.8rem; background: #2a2a2a; border: 1px solid #3a3a3a; color: #e0e0e0; border-radius: 8px; cursor: pointer; text-align: center; transition: all 0.3s ease; font-size: 0.9rem; }
        .chat-question-btn:hover { background: #27ae60; border-color: #27ae60; transform: translateY(-2px); }
        .footer { background: #0a0a0a; border-top: 1px solid #2a2a2a; margin-top: 3rem; padding: 1rem 2rem; }
        ...
        .footer-section a, .footer-section span { color: #888; text-decoration: none; transition: color 0.2s; font-size: 0.85rem; white-space: nowrap; }
